...
<form action='AdminAddSubnetworks.php' method='POST'>
<tr>
<td class='form1'>{$Admin823}:</td>
<td class='form2'><input type='text' class='input-border' name='subnet_name' value='{$subnet_name}' size='40' maxlength='50'></td>
</tr>
<tr>
<td class='form1'>{$Admin824}:</td>
<td class='form2'>
  <table cellpadding='0' cellspacing='0'>
  <tr>
  <td>
  <select class='input-border' style="height:19px; width:36px;">
  ...
  <option></option>
  <option value='=='{if $field1_qual == "=="} SELECTED{/if}>{$Admin826}</option>
  <option value='!='{if $field1_qual == "!="} SELECTED{/if}>{$Admin827}</option>
  <option value='>'{if $field1_qual == ">"} SELECTED{/if}>{$Admin828}</option>
  <option value='<'{if $field1_qual == "<"} SELECTED{/if}>{$Admin829}</option>
  <option value='>='{if $field1_qual == ">="} SELECTED{/if}>{$Admin830}</option>
  <option value='<='{if $field1_qual == "<="} SELECTED{/if}>{$Admin831}</option>
  </select>&nbsp;
  </td>
  <td>
    {* TEXT FIELD *}
...
<td class='form2'>
  ...
  <option></option>
  <option value='=='{if $field2_qual == "=="} SELECTED{/if}>{$Admin826}</option>
  <option value='!='{if $field2_qual == "!="} SELECTED{/if}>{$Admin827}</option>
  <option value='>'{if $field2_qual == ">"} SELECTED{/if}>{$Admin828}</option>
  <option value='<'{if $field2_qual == "<"} SELECTED{/if}>{$Admin829}</option>
  <option value='>='{if $field2_qual == ">="} SELECTED{/if}>{$Admin830}</option>
  <option value='<='{if $field2_qual == "<="} SELECTED{/if}>{$Admin831}</option>
  </select>&nbsp;
  </td>
  <td>
    {* TEXT FIELD *}
...
<td class='form2'>
  <table cellpadding='0' cellspacing='0'>
  <tr>
  <td><label class="button"><input type='submit' value='{$Admin832}'/></label>&nbsp;</td>
  <input type='hidden' name='task' value='doadd'>
  <input type='hidden' name='s' value='{$s}'>
  </form>
  <form action='AdminSubnetworks.php' method='POST'>
  <td><label class="button"><input type='submit' value='{$Admin833}'/></label></td>
  </tr>
  <input type='hidden' name='s' value='{$s}'>
  </form>
  </table>
