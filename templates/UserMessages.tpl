...
<div id="content">
    ...
    <div class="grey-head"><h2>{$Application643}</h2></div>
    <div class="row-blue">
        <p class="blue">{$Application644} {if $user_unread_pms > 0}<b>{/if}{$user_unread_pms} {$Application645} {if $user_unread_pms > 0}</b>{/if}{$Application646}</p>

        {* DISPLAY PAGINATION MENU IF APPLICABLE *}
        {if $maxpage > 1}
        <p class="blue">
            {if $p != 1}<a href='UserMessages.php?p={math equation='p-1' p=$p}'>&#171; {$Application649}</a>{else}<font class='disabled'>&#171; {$Application649}</font>{/if}
            {if $p_start == $p_end}
            &nbsp;|&nbsp; {$Application650} {$p_start} {$Application652} {$total_pms} &nbsp;|&nbsp;
            {else}
            &nbsp;|&nbsp; {$Application651} {$p_start}-{$p_end} {$Application652} {$total_pms} &nbsp;|&nbsp;
            {/if}
            {if $p != $maxpage}<a href='UserMessages.php?p={math equation='p+1' p=$p}'>{$Application653} &#187;</a>{else}<font class='disabled'>{$Application653} &#187;</font>{/if}
        </p>
        {/if}
...
{/if}
        ...
        <div id="ajaxContainer">
            ...
            {if $total_pms == 0}
            <p style="padding-left:25px;">{$Application654}</p>
            <br/>
            ...
            <div id="primary" class="info-cnt tuneddivs" style="color:#666666;">
                <form action='UserMessages.php' method='post' name='messageform' class="settings">
                    ...
                    <div class="f-left">
                        ...
                        {if $total_pms != 0}
                        <a class="button" href="javascript:void(0)" onclick="document.messageform.submit();"><span>{$Application661}</span></a>
